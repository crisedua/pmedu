-- Admin Role Setup for Project Management App (with pm_ prefix)
-- Run this in Supabase SQL Editor if you haven't already

-- Step 1: Check if role column exists
SELECT column_name, data_type, column_default 
FROM information_schema.columns 
WHERE table_name = 'pm_users' AND column_name = 'role';

-- Step 2: Add role column if it doesn't exist
ALTER TABLE pm_users ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'user' 
...
SELECT id, name, email, role FROM pm_users ORDER BY created_at DESC;
